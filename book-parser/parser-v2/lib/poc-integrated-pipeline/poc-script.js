...
//     if (trimmedContent.startsWith(pageNumberStr)) {
//         let cleaned = trimmedContent.substring(pageNumberStr.length);
...
// /**
//  * Remove page numbers from sentence fragments during merging
//  */
// function removePageNumberFromFragment(fragment, pageNumber) {
//     if (!fragment || fragment.length === 0) return fragment;
    
//     const patterns = [
//         new RegExp(`\\s+${pageNumber}\\s+`, 'g'),
//         new RegExp(`${pageNumber}`, 'g'),
//         new RegExp(`\\s+${pageNumber}`, 'g'),
//         new RegExp(`${pageNumber}\\s+`, 'g')
//     ];
    
//     for (const pattern of patterns) {
//         const beforeMatch = fragment.match(new RegExp(`[a-zA-Z]\\s*${pageNumber}\\s*[a-zA-Z]`));
//         if (beforeMatch) {
...
//         let paragraphText = rawParagraphs[i].trim();
...
//         // Remove page numbers from the start of content
//         paragraphText = removePageNumberFromStart(paragraphText, estimatedPage);

//         // Remove page numbers from fragments
//         paragraphText = removePageNumberFromFragment(paragraphText, estimatedPage);
...
//     // Fix incomplete sentences that are split across pages
//     let sentencesMerged = 0;
    
//     // Group paragraphs by chapter for processing
//     const paragraphsByChapter = {};
//     PIPELINE_STATE.paragraphs.forEach(p => {
//         if (!paragraphsByChapter[p.chapterNumber]) {
//             paragraphsByChapter[p.chapterNumber] = [];
//         }
//         paragraphsByChapter[p.chapterNumber].push(p);
//     });

//     // Process each chapter's paragraphs
//     Object.keys(paragraphsByChapter).forEach(chapterNumber => {
//         const chapterParagraphs = paragraphsByChapter[chapterNumber];
        
//         // Sort by page number to ensure correct order
//         chapterParagraphs.sort((a, b) => a.pageNumber - b.pageNumber);
        
//         // Check for incomplete sentences between consecutive paragraphs
//         for (let i = 0; i < chapterParagraphs.length - 1; i++) {
//             const currentParagraph = chapterParagraphs[i];
//             const nextParagraph = chapterParagraphs[i + 1];
            
//             // Skip if not consecutive pages
//             if (nextParagraph.pageNumber - currentParagraph.pageNumber > 1) {
//                 continue;
//             }
...
//             // If current paragraph doesn't end with sentence terminator, try to merge
//             if (!['.', '!', '?', ':', ';'].includes(lastChar)) {
//                 const words = nextText.split(/\s+/);
                
//                 // Look for sentence completion in first few words of next paragraph
//                 for (let wordIndex = 1; wordIndex <= Math.min(20, words.length); wordIndex++) {
//                     const fragment = words.slice(0, wordIndex).join(' ');
                    
//                     if (fragment.match(/[.!?]$/)) {
//                         // Found sentence completion - clean page numbers from the fragment
//                         const cleanedFragment = removePageNumberFromFragment(fragment, nextParagraph.pageNumber);
                        
//                         // Merge the fragment with current paragraph
//                         currentParagraph.text = currentText + ' ' + cleanedFragment;
                        
//                         // Remove the fragment from next paragraph
//                         nextParagraph.text = words.slice(wordIndex).join(' ').trim();
                        
//                         // Update word counts
//                         currentParagraph.wordCount = currentParagraph.text.split(/\s+/).filter(w => w.length > 0).length;
//                         nextParagraph.wordCount = nextParagraph.text.split(/\s+/).filter(w => w.length > 0).length;
...
//     // Remove empty paragraphs that may have been created during merging
//     PIPELINE_STATE.paragraphs = PIPELINE_STATE.paragraphs.filter(p => p.text.trim().length > 0);

//     const debugData = {
//         originalParagraphCount: PIPELINE_STATE.paragraphs.length,
//         sentencesMerged: sentencesMerged,
//         chaptersProcessed: Object.keys(paragraphsByChapter).length,
//         mergedExamples: [] // Could add examples of merged sentences
//     };

//     saveDebugOutput('06', debugData, 'cross-page-merging');

//     console.log(`✓ Processed ${debugData.originalParagraphCount} paragraphs`);
//     console.log(`✓ Merged ${sentencesMerged} sentences split across pages`);
//     console.log(`✓ Chapters processed: ${debugData.chaptersProcessed}`);
