...
// Import the upload functionality from the book-parser directory
const { uploadParsedBook } = require('../../book-parser/upload-book/upload-parsed-book.js');
...
async function uploadTransformersBook() {
    ...
    try {
        ...
        await uploadParsedBook(outputPath);

        console.log('\n✅ Transformers book uploaded successfully!');
        console.log('📖 The book should now be available in the Book Reader App');
        console.log('🔗 Images have been uploaded to Vercel Blob (if token is configured)');
...
async function main() {
    try {
        ...
        if (args.includes('--help') || args.includes('-h')) {
            console.log(`
🧬 Transformers Book Upload Script

Description:
  Uploads the processed Transformers book to the Book Reader App database.
  This script uses the parsed output from run-transformers.js and uploads
  both the book content and images to make them available in the app.

Usage:
  node upload-transformers.js [--force]

Options:
  --force    Force upload even if book already exists in database
  --help     Show this help message

Prerequisites:
  1. Run the parser first: node run-transformers.js
  2. Ensure .env file is configured with MONGODB_URI
  3. Optionally set BLOB_READ_WRITE_TOKEN for image uploads

Examples:
  node upload-transformers.js           # Upload the book
  node upload-transformers.js --force   # Force re-upload
`);
            process.exit(0);
        }
