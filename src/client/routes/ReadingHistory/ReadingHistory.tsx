...
export const ReadingHistory: React.FC = () => {
    ...
    const formatDate = (date: Date): string => {
        ...
        const timeString = sessionDate.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false });
        return timeString;
    };
    ...
    const formatDayHeader = (date: Date): string => {
        ...
        if (diffDays < 7) return sessionDate.toLocaleDateString('en-US', { weekday: 'long' });
    ...
    const groupSessionsByDay = (sessions: ReadingSessionClient[]) => {
        const grouped: { [key: string]: ReadingSessionClient[] } = {};
        ...
        return Object.entries(grouped)
            ...
            .map(([dateKey, daySessions]) => ({
                ...
                sessions: daySessions.sort((a, b) => new Date(b.startTime).getTime() - new Date(a.startTime).getTime()),
    ...
    return (
        <Box sx={{ minHeight: '100vh', backgroundColor: theme.palette.background.default }}>
            ...
            {sessions.length === 0 ? (
                ...
                <Box>
                    {groupSessionsByDay(sessions).map((dayGroup, dayIndex) => (
                        <React.Fragment key={dayGroup.date.toDateString()}>
                            {/* Day Header */}
                            <Box
                                sx={{
                                    px: { xs: 2, sm: 3 },
                                    ...
                                    borderBottom: `0.5px solid ${alpha(theme.palette.divider, 0.2)}`,
                                    borderTop: dayIndex > 0 ? `0.5px solid ${alpha(theme.palette.divider, 0.2)}` : 'none'
                                }}
                            >
                                <Typography
                                    sx={{
                                ...
                                >
                                    {formatDayHeader(dayGroup.date)} (Total: {formatTime(dayGroup.totalDuration)})
                                </Typography>
                            </Box>
                                ...
                                {dayGroup.sessions.map((session, sessionIndex) => (
                                    <React.Fragment key={session.sessionId}>
                                        ...
                                        <ListItem
                                            disablePadding
                                            sx={{
                                                borderBottom: sessionIndex < dayGroup.sessions.length - 1 || expandedSession === session.sessionId
