...
function getVoiceType(voiceId: string, provider: TtsProvider): 'standard' | 'wavenet' | 'neural' | 'neural2' | 'polyglot' | 'studio' | 'chirp3-hd' | 'long-form' | 'generative' {
  if (provider === 'polly') {
...
export const getTtsUsageSummary = async (params?: TtsUsageRangeParams): Promise<TtsUsageSummary> => {
  ...
  const summary: TtsUsageSummary = {
    ...
    usageByProvider: {},
    usageByDay: {},
    freeTierMonthUsage: { polly: { standard: 0, neural: 0, longform: 0, generative: 0 }, google: { standard: 0, wavenet: 0, neural2: 0, polyglot: 0, studio: 0, chirp3hd: 0 }, elevenlabs: { total: 0 } }
  };
