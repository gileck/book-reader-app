 ...
 */
async function extractImages(pdfPath, bookTitle) {
    ...
    const imagesDir = path.join(__dirname, '../public/images', bookFolderName);
...
async function parsePdfWithImages(pdfPath, configPath) {
    ...
    const images = await extractImages(pdfPath, bookMetadata.title);
